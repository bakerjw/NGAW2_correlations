%% script modified to output inter and intra event sigmas
...
% modified by Jack Baker, 8/20/2015 to make the z1 unknown flag = -999
%
...
function [median, sigma, period1, phi_MRV, tau] = BSSA_2014_nga(M, T, Rjb, Fault_Type, region, z1, Vs30)
...
if length (T) == 1 && T == 1000; % Compute median and sigma with pre-defined period
    ...
    for i=1:length(T)
        ...
        if ( isempty(find(abs(period-Ti) < 0.0001))) % The user defined period requires interpolation
            ...
            [Sa_low, sigma_low, phi_MRV_low, tau_low] = BSSA_2014_sub(M, ip_low, Rjb, U, SS, NS, RS, region, z1, Vs30);
            [Sa_high, sigma_high, phi_MRV_high, tau_high] = BSSA_2014_sub(M, ip_high, Rjb, U, SS, NS, RS, region, z1, Vs30);
            ...
            phi_MRV(i) = interp1(x, [phi_MRV_low phi_MRV_high], log(Ti));
            tau(i) = interp1(x, [tau_low tau_high], log(Ti));
        else
            ip_T = find(abs((period- Ti)) < 0.0001);
            [median(i), sigma(i), phi_MRV(i), tau(i)] = BSSA_2014_sub(M, ip_T, Rjb, U, SS, NS, RS, region, z1, Vs30);
        end
    end
end


function [median, sigma, phi_MRV, tau]=BSSA_2014_sub (M, ip, Rjb, U, SS, NS, RS, region, z1, Vs30)
